...
export default function Education3(props){
...
const saveData = () =>{
  updateData("Education3",formValues);
  setIsOpen(false);
}
    return(
      <>
   ...
   >
     ...
     <Modal.Footer>
       ...
       <button className="btn btn-primary btn-sm" onClick={saveData}>Save</button>
        ...
        <div class="detail" style={{height:divHeight+ "px",backgroundColor:bgcolor}}
         onMouseEnter={() => {
          setIsEditShown(true);
          setCurrentDiv("education3")}}
       onMouseLeave={() => {
          setIsEditShown(false)}}
       className={(isEditShown?"hover-border detail":"detail")}>
        <div class="detail-section edu">
          <div class="detail-title">
            <div class="title-icon">
              <i class="fas fa-user-graduate"></i>
            </div>
            <span style={{color:headingcolor, fontSize:headingFont+"px"}}>Education</span>
          </div>
          <div class="detail-content">
          {data.map((element, index) => (
            <div class="timeline-block">
              <h1 style={{color:headingcolor, fontSize:headingFont+"px"}}>{element.degree}</h1>
              <p style={{color:contentcolor,fontSize:contentFont+"px"}}>{element.university}</p>
              <time>{element.time}</time>
            </div>
           ))}
          </div>
        </div>
        {(isEditShown && currentDiv==="education3")?( <button className="btn edit-btn" onClick={showModal}><i class="fas fa-edit"></i></button>):(<div></div>)}
        </div>
        </>
    );
